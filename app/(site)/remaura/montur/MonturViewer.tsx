...
// MONTÜR 3D önizleme — viewer kalıbının montür evi kopyası (taş malzemesi eklendi).
// Varsayılan MAT (form-okuma; ZINCIR.md dersi 2026-07-17: okunurluk > güzellik);
// Parlak yalnız sunum. Renkler ekran ölçümüyle kalibreli tonlardır.
...
export type ViewMesh = {
  ...
  indices: Uint32Array;
  maden?: string;
  /** "tas": kristal malzeme (mat modda da taş — taşın kimliği parlaklıktır) */
  kind?: "metal" | "tas";
};
...
export function MonturViewer({ meshes, maden, fitKey, mod = "mat" }: {
  meshes: ViewMesh[]; maden: string; fitKey?: string; mod?: ViewerMod;
}) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      (window as unknown as Record<string, unknown>).__monturDebug = { scene, camera, renderer, group, controls };
    }
  ...
  useEffect(() => {
    ...
    const malzeme = (m: string) => mod === "mat"
      ? new THREE.MeshStandardMaterial({
          color: MAT_RENK[m] ?? 0x74716c, metalness: 0.0, roughness: 0.9,
    ...
    const tasMat = () => new THREE.MeshPhysicalMaterial({
      color: 0xf4f6fa, metalness: 0, roughness: 0.03,
      transmission: 0.12, ior: 2.417, envMapIntensity: 1.6,
    ...
    for (const m of meshes) {
      ...
      group.add(new THREE.Mesh(geom, m.kind === "tas" ? tasMat() : malzeme(m.maden ?? maden)));
    }
    if (!fittedRef.current && meshes.length && camRef.current && controlsRef.current) {
      ...
      camRef.current.position.set(c.x + size * 0.5, c.y + size * 0.6, c.z + size * 0.9);
    }
    renderNowRef.current?.();
  }, [meshes, maden, mod]);
