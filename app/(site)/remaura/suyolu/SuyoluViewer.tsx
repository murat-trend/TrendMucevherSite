...
// SUYOLU 3D önizleme — komple bileklik montajı (baklalar + taşlar).
...
export function SuyoluViewer({ meshes, maden }: { meshes: ViewMesh[]; maden: string }) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      (window as unknown as Record<string, unknown>).__suyoluDebug = { scene, camera, renderer, group, controls };
    }
  ...
  useEffect(() => {
    ...
    // flatShading: CSG çıktısında düz yüzeyler net okunsun (smooth-normal
    // artefaktı baklaları "buruşuk" gösteriyordu)
    const metalMat = () => new THREE.MeshStandardMaterial({
      color: METAL_RENK[maden] ?? 0xe9c47e, metalness: 0.95, roughness: 0.32,
      envMapIntensity: 0.9, flatShading: true,
    });
    // saydamlık düşük tutulur: koyu zeminde tam saydam taş "boş cep" gibi görünüyordu
    const tasMat = () => new THREE.MeshPhysicalMaterial({
      color: 0xf4f6fa, metalness: 0, roughness: 0.03,
      transmission: 0.12, ior: 2.417, envMapIntensity: 2.2,
    ...
    for (const m of meshes) {
      ...
      group.add(new THREE.Mesh(geom, m.kind === "tas" ? tasMat() : metalMat()));
    }
