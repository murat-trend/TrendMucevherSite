...
export async function POST(req: Request) {
  try {
    ...
    const normalBuffer = await depthToNormalMap(depthBuffer);
    const normalBase64 = normalBuffer.toString("base64");

    return NextResponse.json({
      normalMap: `data:image/png;base64,${normalBase64}`,
    });
  } catch (error: unknown) {
    console.error("NORMAL MAP ERROR:", error);
    const err = error as { message?: string };
    return NextResponse.json(
      { error: err?.message ?? "Normal map üretilemedi." },
      { status: 500 }
    );
  }
}
