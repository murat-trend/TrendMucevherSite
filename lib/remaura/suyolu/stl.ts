// SUYOLU MOTORU — İKİLİ STL YAZICI (bağımsız; suyolu evinin kendi kopyası)
type Mesh = { positions: Float64Array | Float32Array; indices: Uint32Array };

export function toBinarySTL(meshes: Mesh[], name = "Remaura Suyolu"): ArrayBuffer {
  ...
  for (const m of meshes) {
    ...
    for (let k = 0; k < idx.length; k += 3) {
      ...
      off += 14;
      // attribute byte count = 0 (zaten sıfır)
    }
  }
  return buf;
}
