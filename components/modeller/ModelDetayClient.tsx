...
export function ModelDetayClient({ product, sellerId, sellerEmail: initialSellerEmail }: Props) {
  ...
  const [sellerEmail, setSellerEmail] = useState<string | null>(initialSellerEmail)
  ...
  useEffect(() => {
    ...
    const loadSeller = async () => {
      ...
      if (alive) {
        ...
        setSellerAvatar(profile?.avatar_url ?? null)
      }
    }
    void loadSeller()
    return () => { alive = false }
  ...
  const rawGlbHref = product.glbUrl && String(product.glbUrl).trim() !== '' ? String(product.glbUrl).trim() : null
  ...
  const productDisplay = {
    ...
    story: product.story || '',
    price: product.price,
    tags: [product.jewelryType],
    glbFile: slug,
    dimensions: product.dimensions || { width: 22, height: 28, depth: 12 },
    weight: product.weight || 8,
  }
  ...
  useEffect(() => {
    ...
    const bootModelViewer = async () => {
      ...
      if (window.customElements.get('model-viewer')) { if (!cancelled) setViewerReady(true); return }
      ...
      } catch { /* Keep placeholder if definition fails */ }
    }
    void bootModelViewer()
    return () => { cancelled = true }
  }, [])

  const { name: productName, story: productStory } = pickLocalizedProductText(locale, product.translations, {
    name: product.name,
    story: product.story,
    ...
    story_de: product.story_de,
    story_ru: product.story_ru,
  })

  const localizedProduct = {
    ...productDisplay,
    tags: [jewelryTypeLabel(product.jewelryType ?? 'Pandant', locale)],
  }

  const personalPrice =
    product.licensePersonalPrice && product.licensePersonalPrice > 0
      ? product.licensePersonalPrice
      : localizedProduct.price
  const commercialPrice =
    product.licenseCommercialPrice && product.licenseCommercialPrice > 0
      ? product.licenseCommercialPrice
  ...
  return (
    <main style={{ background: '#0a0a0a', color: '#e8e0d0', minHeight: '100vh' }}>
      ...
      >
        ...
        <Link href="/modeller" style={{ fontSize: '11px', color: '#8a8278', letterSpacing: '0.1em', textDecoration: 'none' }}>
      ...
      >
        ...
        <div>
          ...
          >
            <div suppressHydrationWarning style={{ width: '100%', height: '100%' }}>
              {glbUrl && glbReachable === true && viewerReady ? (
                ...
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', textAlign: 'center', color: '#6f6a63', fontSize: '12px', letterSpacing: '0.06em', lineHeight: 1.6 }}>
                  {copy.viewerNoGlb}
                </div>
              ) : (
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6f6a63', fontSize: '12px', letterSpacing: '0.08em' }}>
            ...
            <div style={{ position: 'absolute', bottom: '12px', left: '12px', display: 'flex', gap: '6px' }}>
              {['GLB', 'STL'].map((fmt) => (
                <span key={fmt} style={{ fontSize: '9px', letterSpacing: '0.15em', background: 'rgba(201,168,76,0.08)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.15)', padding: '3px 8px', borderRadius: '2px' }}>
                  {fmt}
                </span>
              ))}
            </div>
          ...
          {/* Yorumlar */}
          <div className="mt-6">
            <h3 className="text-sm font-semibold text-foreground mb-3">Değerlendirmeler ({reviews.length})</h3>
          ...
          {activeImage && (
            <div style={{ marginTop: '8px', background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '2px', overflow: 'hidden', aspectRatio: '1', position: 'relative' }} onClick={() => setActiveImage(null)}>
              <img src={activeImage} alt="Detay" style={{ objectFit: 'cover', width: '100%', height: '100%', display: 'block' }} />
              <span style={{ position: 'absolute', top: '12px', right: '12px', fontSize: '9px', color: '#4a4642', letterSpacing: '0.1em' }}>{copy.close}</span>
            </div>
          )}

          {/* Boyutlar */}
          <div style={{ marginTop: '8px', padding: '1rem', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '2px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
            {[
              { label: copy.dimensions.width, value: `${productDisplay.dimensions.width}mm` },
              { label: copy.dimensions.height, value: `${productDisplay.dimensions.height}mm` },
              { label: copy.dimensions.depth, value: `${productDisplay.dimensions.depth}mm` },
              { label: copy.dimensions.weight, value: `~${productDisplay.weight}g` },
            ].map((item) => (
              <div key={item.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '9px', color: '#4a4642', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '4px' }}>{item.label}</div>
                <div style={{ fontFamily: 'var(--font-serif)', fontSize: '1.1rem', fontWeight: 300, color: '#e8e0d0' }}>{item.value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ paddingTop: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
            {localizedProduct.tags.map((tag) => (
              <span key={tag} style={{ fontSize: '9px', letterSpacing: '0.1em', color: '#4a4642', border: '1px solid rgba(255,255,255,0.07)', padding: '3px 8px', borderRadius: '2px', textTransform: 'uppercase' }}>
                {tag}
              </span>
            ))}
          </div>
          ...
          {/* İsim */}
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '2.8rem', fontWeight: 300, letterSpacing: '0.05em', lineHeight: 1.2, marginBottom: '2rem' }}>
            {productName}
          </h1>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            <div style={{ fontSize: '10px', letterSpacing: '0.2em', color: '#c9a84c', textTransform: 'uppercase', marginBottom: '1rem' }}>{copy.storyTitle}</div>
            <p style={{ color: '#8a8278', lineHeight: 2, fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: '1rem', fontWeight: 300, whiteSpace: 'pre-line' }}>
              {productStory}
            </p>
          </div>

          {/* Ne alacaksın */}
          <div style={{ marginBottom: '2.5rem', padding: '1.25rem', background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '2px' }}>
            <div style={{ fontSize: '10px', letterSpacing: '0.2em', color: '#4a4642', textTransform: 'uppercase', marginBottom: '1rem' }}>{copy.packageTitle}</div>
            {copy.packageItems.map((item) => (
              <div key={item.fmt} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                <span style={{ fontSize: '9px', letterSpacing: '0.1em', background: 'rgba(201,168,76,0.08)', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.15)', padding: '3px 8px', borderRadius: '2px', minWidth: '42px', textAlign: 'center' }}>
                  {item.fmt}
                </span>
          ...
          {/* Lisans seçimi */}
          <div style={{ marginBottom: '12px', padding: '12px', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '2px', background: '#111' }}>
            <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', marginBottom: '10px', color: '#d5cfc5', fontSize: '12px' }}>
          ...
          <p style={{ fontSize: '10px', color: '#4a4642', textAlign: 'center', letterSpacing: '0.1em', lineHeight: 1.8 }}>
            {copy.footer}
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-2xl border border-border/40 bg-[#0f1117] p-6">
            ...
            {messageSent ? (
              ...
              <>
                <textarea value={messageText} onChange={(e) => setMessageText(e.target.value)} placeholder="Mesajınızı yazın..." rows={4} className="w-full rounded-lg border border-border/40 bg-white/[0.04] px-3 py-2 text-sm text-foreground outline-none resize-none" />
                {messageError && <p className="mt-2 text-xs text-red-400">{messageError}</p>}
                <button type="button" onClick={() => void handleSendMessage()} disabled={messageSending || !messageText.trim()} className="mt-3 w-full rounded-lg bg-amber-500/20 border border-amber-500/30 px-4 py-2.5 text-sm font-semibold text-amber-200 hover:bg-amber-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
