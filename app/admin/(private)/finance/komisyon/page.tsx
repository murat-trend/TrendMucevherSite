...
import {
  ...
  Download, RefreshCw, Plus, TrendingUp, Wallet,
} from "lucide-react";
...
// ── Durum badge ───────────────────────────────────────────────────────────
const STATUS_CONFIG: Record<CommissionStatus, { label: string; cls: string; icon: React.ElementType }> = {
...
const EMPTY_FORM: FormState = {
  seller_name: "", sale_amount: "", commission_rate: "8.5",
  due_date: "", notes: "",
};
...
export default function KomisyonPage() {
  ...
  const totalNetToSellers = commissions.filter(c => c.status === "pending" || c.status === "approved")
            ...
            <table className="w-full min-w-[1000px] border-collapse text-left text-sm">
              <thead>
                <tr className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500">
                  {["Fatura No", "Satıcı", "Satış Tutarı", "Komisyon %", "Komisyon", "Net Satıcıya", "Vade", "Durum", "İşlemler"].map(h => (
                    <th key={h} className={`px-4 py-3.5 font-medium ${ADMIN_TABLE_TH_STICKY}`}>{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={row.id} className="transition-colors hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        <button className="rounded-lg border border-white/[0.08] bg-white/[0.04] px-2 py-1 text-[11px] font-medium text-zinc-300 hover:border-white/20">
          ...
          <div className="mx-auto flex min-h-full max-w-lg items-start justify-center py-8">
            <div className="w-full overflow-hidden rounded-2xl border border-white/[0.12] bg-gradient-to-br from-[#12141a] via-[#0d0f15] to-[#08090c] shadow-2xl">
                ...
                {form.sale_amount && form.commission_rate && (
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-sm">
                    ...
                    <div className="mt-1.5 flex justify-between border-t border-white/[0.06] pt-1.5 font-medium text-emerald-400">
                      ...
                      <span className="tabular-nums">{tryFmt(Number(form.sale_amount) * (1 - Number(form.commission_rate) / 100))}</span>
                    </div>
                  </div>
                )}
              ...
              <div className="flex justify-end gap-2 border-t border-white/[0.08] px-6 py-4">
                <button onClick={() => setShowForm(false)} className="rounded-xl border border-white/[0.12] px-4 py-2 text-sm text-zinc-400 hover:border-white/20">
                  Vazgeç
                </button>
                <button
