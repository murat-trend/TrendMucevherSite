...
import { dizilim, rapor, montajMesh, baklaSayisi, BilekRapor } from "@/lib/remaura/suyolu/bileklik";
import { mumaAt, kopyala, duzSiraKonumlar } from "@/lib/remaura/suyolu/mum";
...
export function SuyoluClient() {
  ...
  const [gorunum, setGorunum] = useState<"bilek" | "yerde">("bilek");
  ...
  const kurMeshler = useCallback((b: BaklaSonuc, g: "bilek" | "yerde") => {
    ...
    const tasM = { positions: tPos, indices: t.indices };
    let metal, taslar;
    if (g === "bilek") {
      const { yer } = dizilim(uzunlukMm, capMm);
      metal = montajMesh(b.mesh, yer);
      taslar = montajMesh(tasM, yer);
    } else {
      // yerde: zincir düz açılmış tek sıra
      const { n } = baklaSayisi(uzunlukMm, capMm);
      const konumlar = duzSiraKonumlar(n, b.olculer.adimMm);
      metal = kopyala(b.mesh, konumlar);
      taslar = kopyala(tasM, konumlar);
    }
    setMeshes([
  ...
  }, [busy, capMm, uzunlukMm, maden, tasCinsi, bildir, gorunum, kurMeshler]);

  const gorunumDegistir = (g: "bilek" | "yerde") => {
    setGorunum(g);
    if (bakla) kurMeshler(bakla, g);
  };
  ...
  // MUMA AT: tüm baklalar yere serili + saportlu + raflı — tek dosya yazıcıya
  const stlMumaAt = () => {
    if (!bakla) return;
    const { n } = baklaSayisi(uzunlukMm, capMm);
    const m = mumaAt(bakla.mesh, bakla.olculer.boyMm, bakla.olculer.enMm, n);
    indir(toBinarySTL([m.mesh], "Remaura Suyolu — muma at"),
      `suyolu-mum-${n}bakla-${carat.toFixed(2)}ct.stl`);
    bildir(`Muma at STL indirildi: ${n} bakla ${m.sutun}×${m.satir} · raf ${m.rafMm[0].toFixed(0)}×${m.rafMm[1].toFixed(0)}mm`);
  };
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
              ...
              <div id="suyolu-viewer" className="relative min-h-[440px] overflow-hidden rounded-2xl border border-[#2A2A35] bg-[#101014] lg:col-span-7">
                <SuyoluViewer meshes={meshes} maden={maden} fitKey={gorunum} />
                {busy && (
                ...
                {/* görünüm anahtarı: bilekte (daire) / yerde (düz seriliş) */}
                <div className="absolute right-3 top-3 flex overflow-hidden rounded-lg border border-[#2A2A35] bg-black/50 font-mono text-[11px]">
                  {([["bilek", "Bilekte"], ["yerde", "Yerde"]] as const).map(([id, ad]) => (
                    <button
                      key={id}
