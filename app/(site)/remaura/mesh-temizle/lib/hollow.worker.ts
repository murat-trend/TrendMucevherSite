...
type Req = { positions: Float32Array; wall: number; method: "fast" | "sdf" | "solidify"; maxGrid?: number };

self.onmessage = (e: MessageEvent<Req>) => {
  const { positions, wall, method, maxGrid } = e.data;
  try {
    ...
    if (method === "solidify") {
      const r = solidifyWrap(geo, { onProgress, maxGrid });
    ...
    const r = method === "sdf"
      ? hollowShellSDF(geo, wall, { onProgress, maxGrid })
